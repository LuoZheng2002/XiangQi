...
try:
    ...
    while True:
        ...
        whose_turn = run_dynamic_code(cid_of['func::who_is_next_func'], AGIList([current_chessboard]), cid_of, code_dir)
        ...
        current_chessboard = run_dynamic_code(cid_of['func::operation_func'],
                                              AGIList([current_chessboard, whose_turn]), cid_of, code_dir)
        end_game = run_dynamic_code(cid_of['func::end_game_func'], AGIList([current_chessboard]), cid_of, code_dir)
        if end_game.concept_id == cid_of['True']:
            print_chessboard(current_chessboard, cid_of, cid_reverse)
            print('Game ended!')
            red_benefit = run_dynamic_code(cid_of['func::end_game_benefit'],
                                           AGIList([current_chessboard, obj('xq::red_team', cid_of)]), cid_of, code_dir)
            black_benefit = run_dynamic_code(cid_of['func::end_game_benefit'],
                                             AGIList([current_chessboard,
                                                      obj('xq::black_team', cid_of)]), cid_of, code_dir)
