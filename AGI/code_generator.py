...
def generate_expression(expr: list) -> AGIObject:
    if expr[0] == 'not':
        not_expr = generate_expression(expr[1:])
        return AGIObject(cid_of['dcr::call'],
                         {cid_of['dc::function_name']: obj('func::logic_not'),
    ...
    if ('==' in expr and find_naked_symbol(expr, '==', True) != -1) or \
        ...
        if '==' in expr and find_naked_symbol(expr, '==', True) != -1:
            ...
            function_name = obj('func::compare_concepts')
        elif '===' in expr and find_naked_symbol(expr, '===', True) != -1:
            pos = find_naked_symbol(expr, '===', True)
            function_name = obj('func::math_equal')
        elif '>' in expr and find_naked_symbol(expr, '>', True) != -1:
            pos = find_naked_symbol(expr, '>', True)
            function_name = obj('func::greater_than')
        elif '<' in expr and find_naked_symbol(expr, '<', True) != -1:
            pos = find_naked_symbol(expr, '<', True)
            function_name = obj('func::less_than')
        elif '>=' in expr and find_naked_symbol(expr, '>=', True) != -1:
            pos = find_naked_symbol(expr, '>=', True)
            function_name = obj('func::greater_than_or_equal_to')
        elif '<=' in expr and find_naked_symbol(expr, '<=', True) != -1:
            pos = find_naked_symbol(expr, '<=', True)
            function_name = obj('func::less_than_or_equal_to')
            ...
            if expr[pos] == '+':
                function_name = obj('func::sum')
            else:
                function_name = obj('func::difference')
        elif ('and' in expr and find_naked_symbol(expr, 'and', False) != -1) or \
                ('or' in expr and find_naked_symbol(expr, 'or', False) != -1):
            and_pos = find_naked_symbol(expr, 'and', False)
            or_pos = find_naked_symbol(expr, 'or', False)
            pos = max(and_pos, or_pos)
            if expr[pos] == 'and':
                function_name = obj('func::logic_and')
            else:
                function_name = obj('func::logic_or')
        else:
            assert False
        function_params = list()
        function_params.append(generate_expression(expr[:pos]))
        function_params.append(generate_expression(expr[pos + 1:]))
        return AGIObject(cid_of['dcr::call'],
    ...
    if expr[-1] == ']':
        ...
        return AGIObject(cid_of['dcr::at'],
                         {cid_of['dc::target_list']: generate_expression(expr[:open_square_pos]),
                          cid_of['dc::element_index']: generate_expression(expr[open_square_pos + 1: -1])})
    if find_naked_symbol(expr, '.', False) != -1:
        ...
        if expr[dot_pos + 1] == "'":
            ...
            return AGIObject(cid_of['dcr::get_member'],
                             {cid_of['dc::target_object']: generate_expression(expr[:dot_pos]),
                              cid_of['dc::member_name']: obj(expr[dot_pos + 2])})
        if expr[dot_pos + 1] == 'find' or expr[dot_pos + 1] == 'exist' or expr[dot_pos + 1] == 'count':
            ...
            return AGIObject(concept_id,
                             {cid_of['dc::target_list']: generate_expression(expr[:dot_pos]),
                              cid_of['dc::expression_for_constraint']:
                                  generate_expression(expr[dot_pos + 3:-1])})
        if expr[dot_pos + 1] == 'size':
            assert len(expr) == dot_pos + 2
    ...
    if expr[0] == "'":
        if expr[-1] == "'":
            ...
            return AGIObject(cid_of['dcr::concept_instance'],
                             {cid_of['dc::concept_name']: obj(expr[1])})
        ...
        if expr[-1] == ')':
            ...
            return AGIObject(cid_of['dcr::call'],
                             {cid_of['dc::function_name']: obj(expr[1]),
                              cid_of['dc::function_params']: AGIList(function_params)})
        assert False
    if len(expr) == 1:
    ...
    while line_index < len(block):
        ...
        if '=' in line_pieces or '&=' in line_pieces:
            ...
            line_object = AGIObject(cid,
                                    {cid_of['dc::left_value']: generate_expression(line_pieces[:equal_pos]),
                                     cid_of['dc::right_value']: generate_expression(line_pieces[equal_pos + 1:]),
                                     cid_of['dc::line_index']: num_obj(real_line_index[0])})
            real_line_index[0] += 1
            lines.append(line_object)
            line_index += 1
        elif line_pieces[0] == 'return':
            line_object = AGIObject(cid_of['dcr::return'],
                                    {cid_of['dc::return_value']: generate_expression(line_pieces[1:]),
                                     cid_of['dc::line_index']: num_obj(real_line_index[0])})
            real_line_index[0] += 1
            lines.append(line_object)
            line_index += 1
            ...
            line_object = AGIObject(cid_of['dcr::assert'],
                                    {cid_of['dc::assert_expression']: generate_expression(line_pieces[1:]),
                                     cid_of['dc::line_index']: num_obj(real_line_index[0])})
            real_line_index[0] += 1
            lines.append(line_object)
            line_index += 1
            ...
            if iter_id_str in letter_to_number.keys():
                iter_id = num_obj(letter_to_number[iter_id_str])
            else:
                assert iter_id_str[:4] == 'iter'
                assert iter_id_str[4:].isdigit()
                iter_id = num_obj(int(iter_id_str[4:]))
            ...
            line_object = AGIObject(cid_of['dcr::for'],
                                    {cid_of['dc::iterator_index']: iter_id,
                                     ...
                                     cid_of['dc::for_block']: generate_block(for_block, indentation_count + 1, True, real_line_index),
                                     cid_of['dc::line_index']: num_obj(for_statement_line_index)})
            lines.append(line_object)
            ...
            line_object = AGIObject(cid_of['dcr::while'],
                                    {cid_of['dc::expression_for_judging']: expression_for_judging,
                                     ...
                                     cid_of['dc::line_index']: num_obj(while_statement_line_index)})
            lines.append(line_object)
        elif line_pieces[0] == 'break':
            assert len(line_pieces) == 1
            line_object = AGIObject(cid_of['dcr::break'],
                                    {cid_of['dc::line_index']: num_obj(real_line_index)})
            real_line_index[0] += 1
            lines.append(line_object)
            line_index += 1
            ...
            if line_index != len(block):
                while block[line_index].line_str[:4] == 'elif':
                    ...
                    elif_module = AGIObject(cid_of['dc::elif_module'],
                                            {cid_of['dc::expression_for_judging']: elif_expression,
                                             ...
                                             cid_of['dc::line_index']: num_obj(elif_statement_line_index)})
            ...
            line_object = AGIObject(cid_of['dcr::if'],
                                    {cid_of['dc::expression_for_judging']: expression_for_judging,
                                     ...
                                     cid_of['dc::else_block']: generate_block(else_block, indentation_count + 1, True, real_line_index),
                                     cid_of['dc::line_index']: num_obj(if_statement_line_index)})
            lines.append(line_object)
            ...
            if is_append:
                line_object = AGIObject(cid_of['dcr::append'],
                                        {cid_of['dc::target_list']: generate_expression(line_pieces[:pos - 1]),
                                         cid_of['dc::element']: generate_expression(line_pieces[pos + 2:-1]),
                                         cid_of['dc::line_index']: num_obj(real_line_index[0])})
            else:
            ...
            for piece in sub_pieces:
                if piece != ',':
                    ...
                    register_indices.append(num_obj(int(piece[3:])))
            ...
            line_object = AGIObject(cid_of['dcr::request'],
                                    {cid_of['dc::requested_registers']: AGIList(register_indices),
                                     ...
                                     cid_of['dc::provided_block']:
                                         generate_block(provided_block, indentation_count + 1, True, real_line_index),
                                     cid_of['dc::line_index']: num_obj(request_statement_line_index)})
            lines.append(line_object)
        elif line_pieces[0] == "'" and line_pieces[-1] == ')':
            assert line_pieces[2] == "'"
            assert line_pieces[3] == '('
            code_object = obj(line_pieces[1])
            sub_pieces = line_pieces[4:-1]
            code_params = list()
            if sub_pieces:
                while find_naked_symbol(sub_pieces, ',', True) != -1:
                    comma_pos = find_naked_symbol(sub_pieces, ',', True)
                    code_params.append(generate_expression(sub_pieces[:comma_pos]))
                    sub_pieces = sub_pieces[comma_pos + 1:]
                if sub_pieces:
                    code_params.append(generate_expression(sub_pieces))
...
def generate_code(code_name: str):
